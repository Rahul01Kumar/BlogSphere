...
pipeline {
    ...
    environment {
        IMAGE_NAME = 'blogsphere-backend'  // You can customize this
        CONTAINER_NAME = 'blogsphere-container'
        APP_PORT = '3001'
    }
    ...
    stages {
        stage('Clone Repo') {
            steps {
                git branch: 'main', url: 'https://github.com/Rahul01Kumar/BlogSphere.git'
            }
        }

        stage('Install Dependencies') {
            steps {
                dir('backend') {   // move inside backend folder
                    sh 'npm install'
                }
            }
        }
        ...
        stage('Inject .env file') {
            steps {
                script {
                    def envVars = """
                    PORT=3001
                    MONGODB_URI=mongodb+srv://yadav28rahul10:kutta12@cluster0.nqioiwi.mongodb.net/
                    """
                    writeFile file: 'backend/.env', text: envVars
                }
            }
        }
        ...
        stage('Docker Run') {
            steps {
                script {
                    // Clean up any previous running container
                    sh "docker rm -f $CONTAINER_NAME || true"
    ...
    post {
        success {
            echo '✅ Application deployed successfully inside Docker!'
        }
        failure {
